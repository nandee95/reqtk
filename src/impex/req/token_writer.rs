...
impl TokenWriter {
    pub fn write_location(
        ...
        if let Location::Path(_) = location {
            let size = writer.stream_position().err_localized(location)?;
            writer.set_len(size).err_localized(location)?;
        }
        Ok(())
    }
        ...
        W: Write,
    {
        match formatting {
            ...
            Formatting::Reformat => Self::write_reformat(&mut writer, &tokens),
    ...
    pub fn write_reformat<'a, W, T>(writer: &mut W, tokens: &'a [T]) -> Result<(), IoError>
    where
        W: Write,
